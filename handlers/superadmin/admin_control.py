...
from keyboards.inline.superAdminKeyboards import cancel, menu_super_admin, done
...
async def add_admin(call: CallbackQuery, state: FSMContext):
    await call.answer(cache_time=1)
    await call.message.edit_text("✍️ Yangi admin ismini kiriting:", reply_markup=cancel)
...
https://t.me/{}?start={}
""".format((await dp.bot.me).username, otp), reply_markup=done)
...
async def admin_control(call: CallbackQuery):
    ...
    await call.message.edit_text("⚙️ Adminni boshqarish:", reply_markup=InlineKeyboardMarkup(inline_keyboard=btn))
